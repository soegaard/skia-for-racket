...
int main(void) {
    printf("host C: image-info=%zu rect=%zu point=%zu irect=%zu sampling=%zu png-options=%zu jpeg-options=%zu webp-options=%zu font-metrics=%zu\n",
           sizeof(image_info), sizeof(rect), sizeof(point), sizeof(irect), sizeof(sampling),
           sizeof(png_options), sizeof(jpeg_options), sizeof(webp_options), sizeof(font_metrics));
